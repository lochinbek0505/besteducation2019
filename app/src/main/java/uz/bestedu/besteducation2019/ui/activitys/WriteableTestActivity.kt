...
import uz.bestedu.besteducation2019.databinding.ActivityWriteableTestBinding
                    ...
                    Log.e(
                        "ASDFG", "${
                            binding.textInputEditText.text.toString()
                                .toLowerCase().equals(
                                    tests.get(index).json.answers.get(0).value1.toLowerCase()
                                        .toString(), false
                                )
                        }"
                    )

                    val responseString = binding.textInputEditText.text.toString().trim()
                    val responseString2 = tests.get(index).json.answers.get(0).value1.toString().trim()
//                    Toast.makeText(this, responseString + " " + responseString2, Toast.LENGTH_SHORT)
//                        .show()
                    if (responseString.equals(
                            responseString2, false
                        )
                    ) {
                        ball++
...
//                    response = Answer("", "", false)
                    ...
                    val responseString = binding.textInputEditText.text.toString().trim()
                    val responseString2 = tests.get(index).json.answers.get(0).value1.trim()
                    if (responseString.equals(
                            responseString2, false
                        )
                    ) {
